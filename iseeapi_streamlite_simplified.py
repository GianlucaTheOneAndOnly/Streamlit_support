...
class Api:
    ...
    def loginAPI(self, username, password, server):
        """
        Stores credentials and fetches available databases without creating a token yet.
        """
        st.session_state.username = username
        st.session_state.password = password
        st.session_state.urlserver = self.SERVER_SUFFIX[server]
        self.session = requests.Session()

        # Auth just for database listing (uses Basic Auth)
        url = f"https://isee{st.session_state.urlserver}.icareweb.com/apiv4/databases/"
        response = self.session.get(url, auth=(username, password))

        if response.ok:
            st.session_state.logged_in = True
            st.session_state.dbs = response.json()
            return True
        else:
    ...
    def create_token(self):
        """
        Create and store a token after a database has been selected.
        """
        if not hasattr(self, 'session'):
            self.session = requests.Session()

        if 'username' not in st.session_state or 'password' not in st.session_state or 'database' not in st.session_state:
            st.error("Missing credentials or database selection.")
            return False

        url = f"https://isee{st.session_state.urlserver}.icareweb.com/apiv4/{st.session_state.database}/token/"
        response = self.session.post(url, auth=(st.session_state.username, st.session_state.password))

        if response.ok:
            token = response.json().get("token")
            self.session.headers.update({"Authorization": f"Token {token}"})
            st.session_state.session = self.session
            return True
        else:
            st.error(f"❌ Token creation failed: {response.status_code} - {response.text}")
            return False


    def select_database(self, db_name):
        """
        Store selected database ID and generate token.
        """
        if 'dbs' not in st.session_state:
            st.error("No database list found. Please login first.")
            return False

        selected_db = next((db for db in st.session_state['dbs'] if db['name'] == db_name), None)
        if selected_db is None:
            st.error("Selected database not found.")
            return False

        st.session_state.database = selected_db['id']

        # Create token after selecting the DB
